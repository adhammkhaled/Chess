...
import static backend.ChessCore.PawnPromotion.Bishop;
import static backend.ChessCore.PawnPromotion.Knight;
import static backend.ChessCore.PawnPromotion.Rook;
...
public abstract class ChessGame { 
    //ChessGame class will as caretaker for memento
    ...
    private Stack<ChessGameMemento> mementoStack = new Stack<>();
    private boolean justUndoneMove = false;
    private boolean UndoToInitialState = true;
    private ChessGameMemento initialState;
    ...
    public boolean makeMove(Move move) {
        ...
        lastMove = move;
        updateGameStatus();
        this.justUndoneMove=false;
        saveMemento();
        this.UndoToInitialState=false;
        return true;
    }
    ...
    public ChessGameMemento createMemento() {
        return new ChessGameMemento(
                new ChessBoard(board), gameStatus, whoseTurn,
                canWhiteCastleKingSide, canWhiteCastleQueenSide,
                canBlackCastleKingSide, canBlackCastleQueenSide, lastMove);
    }
    public void restoreFromMemento(ChessGameMemento memento) {
        this.board.copyFrom(memento.getBoardSnapshot());
        this.gameStatus = memento.getGameStatus();
        this.whoseTurn = memento.getWhoseTurn();
        this.canWhiteCastleKingSide = memento.getIsCanWhiteCastleKingSide();
        this.canWhiteCastleQueenSide = memento.getIsCanWhiteCastleQueenSide();
        this.canBlackCastleKingSide = memento.getIsCanBlackCastleKingSide();
        this.canBlackCastleQueenSide = memento.getIsCanBlackCastleQueenSide();
    ...
    public boolean undoLastMove() {
        if (!mementoStack.isEmpty()) {
            if(this.justUndoneMove==false)
                mementoStack.pop();
            if(!mementoStack.isEmpty()){
                ChessGameMemento previousMemento = mementoStack.pop();
                restoreFromMemento(previousMemento);
            }
            else{
                restoreFromMemento(initialState);
                this.UndoToInitialState=true;
                return true;
            }
            this.justUndoneMove=true;
            
            return true;
        }
        else if(this.UndoToInitialState==false){ //to not flip the board with initial state
            restoreFromMemento(initialState);
            this.UndoToInitialState=true;
            return true;
        }
